...
function updateChart() {
  ...
  // 分离x轴和y轴数据，将布尔值转换为0/1
  const xData = dataPoints.map(point => formatTimestamp(point[0]));
  const yData = dataPoints.map(point => point[1] ? 1 : 0);
  ...
  const options = {
    ...
    tooltip: {
      ...
      formatter: (params: any) => {
        ...
        return `${param.name}<br />${displayTitle.value}: ${param.value === 1 ? '开启' : '关闭'}`;
      }
    },
    grid: {
      left: '3%',
    ...
    series: [
      {
        ...
        itemStyle: {
          color: (params: any) => {
            return params.value === 1 ? '#67c23a' : '#f56c6c';
          }
        }
      }
    ]
...
<template>
  <div class="bool-chart-container">
    <NDivider v-if="title">
